...
2. **Tag and push it.**
   ...
   That is the whole trigger. Pushing to a branch publishes nothing:
   `release-platform.yml` runs on `v*.*.*` tags and on
   `workflow_dispatch` naming an existing tag, and nothing else. A
   pre-release tag (`v0.4.0-rc.1`) is refused — `latest` and `appVersion`
   both mean a plain release.
3. **The workflow publishes.** Per image it builds the candidate with no
   registry credentials in the job yet, checks it runs as non-root and
   fits its size ceiling, and only then logs in and pushes — a cache
   replay of the build that was checked, so a failed gate means nothing
   is published. Each pushed digest is then signed with cosign (keyless)
   and gets a signed SLSA provenance attestation, alongside BuildKit's
   own provenance and SBOM.

   ```sh
   # What was published, and by which workflow run:
   cosign verify ghcr.io/felixgeelhaar/glossa-server@sha256:… \
     --certificate-identity-regexp \
       '^https://github.com/felixgeelhaar/glossa/.github/workflows/release-platform.yml@' \
     --certificate-oidc-issuer https://token.actions.githubusercontent.com
   gh attestation verify oci://ghcr.io/felixgeelhaar/glossa-server@sha256:… \
     --repo felixgeelhaar/glossa
   ```

4. **Take the digests.** The run's `values-images.yaml` artifact holds
   the three digests it published, as chart values (it is also printed in
   the run summary):

   ```sh
   gh run download --repo felixgeelhaar/glossa <run-id> -n values-images
   ```

   ```yaml
   # Generated by .github/workflows/release-platform.yml for v0.4.0.
   image:
     requireDigest: true
   server:
     image:
       ...
       digest: sha256:…
   edge:
     image: { tag: "0.4.0", digest: sha256:… }
   studio:
     image: { tag: "0.4.0", digest: sha256:… }
   ```

   The file is **not** committed. A checked-in copy goes stale the moment
   an image is republished, and the point of pinning is that the values
   and the registry cannot drift apart; the artifact is generated by the
   run that did the publishing. `examples/values-klarlabs.yaml` therefore
   keeps its `digest: ""` placeholders and carries the hosts, storage and
   backup decisions only — the digests come on top of it, as a second
   `-f`. Regenerate the same file later (for an older release, or if the
   artifact has expired) with
   [`ci/image-digests.sh`](ci/image-digests.sh):
...
5. **Deploy.** The digests file goes last, so it wins:
   ...
   helm upgrade --install glossa-platform deploy/charts/glossa-platform \
     ...
     -f deploy/charts/glossa-platform/examples/values-klarlabs.yaml \
     -f values-images.yaml \
     --timeout 15m
   ```

   **The first install runs without `--wait`** — the database hooks
   bootstrap the app login and the server pods cannot become ready before
   they finish. See [First install](#first-install) step 7 and
   [Order of the database hooks](#order-of-the-database-hooks) for why,
   and for the `kubectl rollout status` that follows it. Later upgrades
   take `--wait`. Klarlabs rolls out through RollOps instead; see
   [Deploying with RollOps](#deploying-with-rollops), which needs the
   same digests.

`workflow_dispatch` with the tag name republishes a version — after a
failed run, say. It produces exactly what the tag push would, so the
digests may change even though the version does not; re-take them.

The chart itself is not published to an OCI registry yet: install it from
...
`values-images.yaml` is the release run's artifact, or `ci/image-digests.sh
<tag>` ([Cutting a release](#cutting-a-release)). It sets
`image.requireDigest`, so a render that would produce an unpinned
workload fails instead of generating one.
...
| `server.purge.enabled` | `true` | `GLOSSA_PURGE_ENABLED`: the daily jobs — builds beyond retention (the latest 3 per application, branch and source, and a closed branch's after 7 days) with their captures, the images no capture references, and Catalog's separate sweep of the proposals of branches closed 14 days ago. One replica leads each run (a lease), so leave it on everywhere. |

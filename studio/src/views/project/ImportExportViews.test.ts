...
const conflictResults = (): Array<Omit<ImportResult, "seq">> => [
  { kind: "translation", key: "checkout.title", locale: "de", status: "created" },
  { kind: "translation", key: "cart.items", locale: "de", status: "conflict", code: "approved_translation_conflict", line: 3, column: 5 },
...
describe("ImportView", () => {
  it("recognizes an XLIFF file, starts with a dry run and lands on its results", async () => {
    ...
    expect(w.get("[data-testid=xliff-target]").text()).toBe("de, from the file (trgLang).");
    ...
    expect(fake.calls.find((c) => c[0] === "createImport")![1]).toEqual({ format: "xliff", mode: "dry_run", file_name: "de.xlf", project_id: "p" });
  ...
  it("reads a PO file's Language header and imports TMX tenant-wide", async () => {
    const fake = createFakeIntegration();
    const w = await screen(ImportView, fake);
    ...
    await w2.get("input[name=imp-scope][value=tenant]").setValue(true);
    await w2.get("form").trigger("submit");
    await until(() => expect(fake2.calls.find((c) => c[0] === "createImport")?.[1]).toEqual({ format: "tmx", mode: "dry_run", file_name: "memory.tmx" }));
  });
});
...
describe("ImportExportView", () => {
  ...
  it("exports JSON for two locales and downloads the zip", async () => {
    const fake = createFakeIntegration();
    ...
    expect(fake.calls.find((c) => c[0] === "createExport")![1]).toEqual({
      project_id: "p",
      format: "json",
      options: { locales: ["en", "de"], states: ["approved", "needs_review"], layout: "flat", syntax: "mf1" },
    });
  ...
  it("exports the translation memory workspace-wide, and opens the wizard with i", async () => {
    const fake = createFakeIntegration();
    const w = await screen(ImportExportView, fake, { path: "/t/t/p/p/files" });
    await button(w, "Export translation memory (TMX)").trigger("click");
    const dialog = w.get("dialog[open]");
    expect(dialog.get("h2").text()).toBe("Export translation memory");
    await dialog.get("input[name=exp-scope][value=tenant]").setValue(true);
    await dialog.get("#exp-source").setValue("en");
    await dialog.get("form").trigger("submit");
    await until(() => expect(fake.calls.find((c) => c[0] === "createExport")?.[1]).toEqual({ format: "tmx", options: { source_locale: "en" } }));

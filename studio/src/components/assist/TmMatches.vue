...
const props = defineProps<{ tenant: string; projectId: string; message: Message; source: ProjectLocale; target: ProjectLocale; canInsert: boolean }>();
...
watch(() => [props.message.id, props.message.source_revision, props.target.code], load, { immediate: true });
...
function matchTarget(n: number): string | undefined {
  return matches.value[n - 1]?.target;
}
    ...
    <ol v-else class="matches">
      <li v-for="(m, i) in matches" :key="m.unit.id" class="match stack-sm" data-testid="tm-match">
        <div class="row">
          ...
          <span class="spacer" />
          <button type="button" class="btn btn-sm" :disabled="!canInsert" :aria-keyshortcuts="i < 9 ? ariaKeys(['Mod', 'Alt', String(i + 1)]) : undefined" @click="emit('insert', m.target)">
            {{ s.insert }} <span v-if="i < 9" class="kbd-hint" aria-hidden="true">{{ chord(i) }}</span>
          </button>
        </div>
        <p class="target" :lang="target.code" :dir="target.direction">{{ m.target }}</p>

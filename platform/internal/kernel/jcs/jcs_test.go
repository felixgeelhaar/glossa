...
// Vectors from RFC 8785 §3.2.2.3 and Appendix B (shared with the Go
// runtime's verifier, runtimes/go/jcs_test.go).
...
func TestFormatNumber(t *testing.T) {
	...
	for bits, want := range cases {
		...
		if err != nil || got != want {
			t.Errorf("formatNumber(%#x) = %q, %v; want %q", bits, got, err, want)
		}
	}
	for _, bad := range []float64{math.NaN(), math.Inf(1), math.Inf(-1)} {
		if _, err := formatNumber(bad); err == nil {
			t.Errorf("formatNumber(%v) succeeded; want an error", bad)
		}
	}
}
...
func TestCanonicalizeRejects(t *testing.T) {
	for _, in := range []string{
		...
		if got, err := Canonicalize([]byte(in)); err == nil {
			t.Errorf("Canonicalize(%q) = %s; want an error", in, got)
		}
	}
}

func TestWithout(t *testing.T) {
	got, err := Without([]byte(`{"z":1,"signatures":[{"sig":"x"}],"a":{"signatures":2}}`), "signatures")
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarshal(t *testing.T) {
	...
	got, err := Marshal(map[string]any{"b": inner{Z: "<x>", A: 2}, "a": []int{3, 1}})
	if err != nil {
		t.Fatal(err)
	}
	if want := `{"a":[3,1],"b":{"a":2,"z":"<x>"}}`; string(got) != want {
		t.Fatalf("got %s, want %s", got, want)
	}
}

func TestIdempotent(t *testing.T) {
	once, err := Canonicalize([]byte(`{"b":"\u00e9","a":[1.0,2.50]}`))
	if err != nil {
		t.Fatal(err)
	}
	twice, err := Canonicalize(once)
	if err != nil || string(once) != string(twice) {
		t.Fatalf("canonical form is not a fixed point: %s vs %s (%v)", once, twice, err)
	}
}

...
-- migration opens just enough to glossa_system for the scheduler to
-- find the tenants that have such messages, and nothing else: branch
-- states and timestamps, proposal links, message states. No text, no
-- keys.

CREATE POLICY catalog_branches_system_select ON catalog_branches
    FOR SELECT TO glossa_system USING (true);
GRANT SELECT (tenant_id, id, state, closed_at) ON catalog_branches TO glossa_system;

CREATE POLICY catalog_proposals_system_select ON catalog_proposals
    FOR SELECT TO glossa_system USING (true);
GRANT SELECT (tenant_id, branch_id, message_id, kind) ON catalog_proposals TO glossa_system;
...
-- The scheduler's lookup: proposed messages whose every proposing
